...
import {
  ...
  Calendar, LayoutGrid, FileText, MapPin, ChevronRight, Camera, Loader2
} from 'lucide-react';
...
const ClientProfilePage: React.FC = () => {
    ...
    const { data, isLoading: loadingData } = useClientProfileData(user?.id);
    ...
    const handleProfileUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const { error } = await supabase.from('profiles').upsert({
                ...
                address: profileData.address, // Guardando o objeto consolidado
                        ...
                        <div className="sticky top-24 space-y-6">
                            <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-8 border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] text-center relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-center gap-1.5 text-xs font-bold text-slate-500 mb-6 bg-slate-50 py-2.5 px-4 rounded-xl border border-slate-100">
                                    ...
                                    {data?.profile?.address?.city || 'Localização Pendente'}
                                </div>
                                ...
                                <div className="bg-white rounded-[2.5rem] shadow-xl p-8 border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <form onSubmit={handleProfileUpdate} className="space-y-8">
                                        ...
                                        <div className="flex justify-end pt-4">
                                            <Button type="submit" isLoading={loadingData} className="px-10 rounded-2xl font-black text-sm uppercase tracking-widest h-14 shadow-lg shadow-brand-primary/20">
                                                Atualizar Perfil
                                            </Button>
                                        </div>
                                    </form>
