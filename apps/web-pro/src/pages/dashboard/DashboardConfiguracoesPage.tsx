...
type TabKey = 'geral' | 'assinatura' | 'notificacoes' | 'seguranca' | 'horarios' | 'danger';

const TABS: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  { key: 'geral', label: 'Geral', icon: <Settings size={14} /> },
  { key: 'assinatura', label: 'Assinatura', icon: <CreditCard size={14} /> },
        ...
        >
          ...
          {activeTab === 'assinatura' && (
            <div className="space-y-6 max-w-2xl">
              <div>
                <h3 className="text-sm font-bold text-gray-800">Seu Plano Atual</h3>
                <p className="text-xs text-gray-400 mt-0.5">Gerencie seu plano, faturas e metodos de pagamento</p>
              </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Status: <span className="text-emerald-600 font-bold uppercase">{company?.subscription_status || 'Ativo'}</span>
                    </p>
                  </div>
                </div>
                
                <a
                  href={`/dashboard/empresa/${company?.slug}/assinatura`}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-gray-900 text-white text-xs font-bold rounded-xl hover:bg-gray-800 transition-all hover:shadow-lg"
                >
                  <CreditCard size={16} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-gray-100 bg-gray-50/50">
                  <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Cobranca</p>
                  <p className="text-xs text-gray-700 font-medium">Os pagamentos sao processados via Stripe de forma segura.</p>
                </div>
                <div className="p-4 rounded-xl border border-gray-100 bg-gray-50/50">
                  <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Duvidas?</p>
                  <a href="#" className="text-xs text-primary-600 font-medium hover:underline">Fale com nosso suporte</a>
                </div>
              </div>
            </div>
          )}
