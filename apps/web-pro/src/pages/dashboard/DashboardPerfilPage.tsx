...
const calculateCompletion = (company: CompanyData | null, formData: FormState): CompletionResult => {
  ...
  const fields = [
    ...
    { condition: !!formData.terms_and_policies && formData.terms_and_policies.trim().length > 0, label: 'Termos e Políticas', tab: 'termos' as TabKey },
  ];
...
const DashboardPerfilPage: React.FC = () => {
  ...
  useEffect(() => {
    if (company) {
      const data: FormState = {
        ...
        coverage_radius_km: company.coverage_radius_km ?? 30,
        coverage_neighborhoods: company.coverage_neighborhoods || [],
        terms_and_policies: company.terms_and_policies || '',
        slug: company.slug || '',
      };
      setFormData(data);
  ...
  // ─── Slug Editable Logic ───────────────────────────────────────────────────
  const { isSlugEditable, daysRemaining } = useMemo(() => {
    if (!company?.slug_updated_at) return { isSlugEditable: true, daysRemaining: 0 };
    
    const lastUpdate = new Date(company.slug_updated_at);
    const now = new Date();
    const diffDays = Math.ceil((now.getTime() - lastUpdate.getTime()) / (1000 * 60 * 60 * 24));
    
    return {
      isSlugEditable: diffDays >= 60,
      daysRemaining: Math.max(0, 60 - diffDays)
    };
  }, [company?.slug_updated_at]);
  ...
  const handleCropConfirm = useCallback(async (croppedFile: File) => {
    ...
    try {
      ...
      // Convert file to base64 for IA analysis
      const reader = new FileReader();
      ...
      const { data: analysis, error: aiError } = await supabase.functions.invoke('analyze-image', {
        body: { image: base64Image, type: cropType }
      });
      ...
      // Fail open but notify
      addToast('Aviso: Não foi possível analisar a imagem via IA, mas prosseguindo com o upload.', 'info');
      setCropModal(prev => ({ ...prev, isOpen: false }));
      if (cropType === 'avatar') {
        uploadAvatar(croppedFile);
      } else {
  ...
  const saveProfile = async () => {
    try {
      ...
      const { error } = await supabase.from('companies').update({
        ...
        slug: formData.slug,
        slug_updated_at: formData.slug !== company?.slug ? new Date().toISOString() : company?.slug_updated_at,
      }).eq('id', company?.id);
  ...
  return (
    <div className="max-w-5xl mx-auto pb-28 space-y-5 sm:space-y-6">
          ...
          {activeTab === 'dados' && (
            <div className="space-y-8">
              ...
              >
                ...
                <div className="pt-2">
                  <Input
                    label="Nome de Usuário (Slug da URL)"
                    name="slug"
                    value={formData.slug || ''}
                    onChange={handleChange}
                    disabled={!isSlugEditable}
                    placeholder="ex: minha-empresa"
                    helperText={!isSlugEditable 
                      ? `Você poderá alterar novamente em ${daysRemaining} dias.` 
                      : "Este será o seu endereço público na TGT (tgt.com.br/empresa/seu-nome)"
                    }
                    prefix="tgt.com.br/empresa/"
                  />
                </div>
                <Select
      ...
      <ImageCropModal
        ...
        onClose={() => !isAnalyzing && setCropModal(prev => ({ ...prev, isOpen: false }))}
