...
const targetFile = 'f:/MVP/test 4/tgt-contratto-mvp/apps/web/src/pages/CompanyProfilePage.tsx';
let content = fs.readFileSync(targetFile, 'utf8');

// 1. Remove activeTab useState
content = content.replace(
  /const \[activeTab, setActiveTab\] = useState\<'services' \| 'about' \| 'reviews'\>\('services'\);\n/g,
  ''
);

// 2. Replace the Tabs container logic
const startPattern = /\{\/\* Tabs \*\/\}\s*<div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6 overflow-hidden">/;
const startMatch = content.match(startPattern);

if (startMatch) {
  const startIndex = startMatch.index;
  // Find the exact end of the block </div>\n            </div>\n\n            {/* Similar Companies */}
  const endPattern = /\{\/\* Similar Companies \*\/\}/;
  const endMatch = content.match(endPattern);
  
  if (endMatch) {
    // we go backward to the last </div> before Similar Companies
    const beforeSimilarIndex = content.lastIndexOf('</div>\n            </div>', endMatch.index);
    if (beforeSimilarIndex > startIndex) {
      const endIndex = beforeSimilarIndex + '</div>\n            </div>'.length;
      
      const newBlock = `{/* Profile Content */}
